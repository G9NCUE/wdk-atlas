...
export function originFor(metric, ctx = {}) {
  ...
  if (metric.kind === "seriesTotal") {
    return org ? { label: "GitHub pull requests", href: api(`search/issues?q=org:${org}+is:pr+is:merged`) } : null;
  }
  if (metric.kind === "currency") {
    return pkg ? { label: "npm versions", href: `https://api.npmjs.org/versions/${encodeURIComponent(pkg)}/last-week` } : null;
  }
...
export const uncovered = (metrics) => (metrics || []).filter((m) => !originFor(m, PROBE)).map(describe);
