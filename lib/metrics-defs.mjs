...
//   tip       one line for the hover, at most TIP_MAX characters; the definition is for the export
...
export const REQUIRED = ["id", "label", "tip", "question", "definition", "source", "window", "decision", "canFall", "verdict"];
/** The most a hover may say. The definition is the contract and travels with every export; the
 * tip is what fits under a cursor, and a tile that needs a paragraph to be read is not a tile. */
export const TIP_MAX = 80;

/** Every entry's verdict, checked against the list above at load so a typo cannot silently
 * drop a metric out of DEMOTED and quietly un-demote it. */
const checkVerdicts = (defs) => { for (const d of defs) if (!VERDICTS.includes(d.verdict)) throw new Error(`metrics-defs: ${d.id} has an unknown verdict "${d.verdict}"`); return defs; };

/** Verdicts from the measurement review. `keep` needs no work; the rest are backlog. */
const VERDICTS = ["keep", "redefine", "promote", "demote", "move", "add"];

const defs = [
  // ---- Try: did someone start? ------------------------------------------------------------
  {
    id: "downloads",
    tip: "Tarball fetches of the selected packages, last complete period.",
    label: "npm downloads",
  ...
  {
    id: "downloads-direct",
    tip: "Installs no WDK dependency pin explains. A lower bound.",
  ...
  },
  {
    ...
    tip: "Installs pulled in by another WDK package's pin. An upper bound.",
  ...
  {
    id: "dependents",
    tip: "Public repos outside the org that name a WDK package.",
  ...
  },
  {
    ...
    tip: "Dependents, forks aside, committed to in the last 90 days.",
  ...
  {
    id: "version-currency",
    tip: "Share of chosen installs on npm's latest release, last week.",
    label: "Version currency",
    question: "Are the people who installed WDK still keeping up with it?",
    definition:
      "Share of a package's installs landing on the version npm serves as latest. Reported beside the share landing on any release under 30 days old, which is a different question: a package we have not released for a month scores nothing on that one however faithfully people upgrade.",
    source: "npm registry per-version downloads",
    window: "last week of downloads, against the dist-tags and the release dates",
    floor: 100,
    canFall: true,
    decision:
      "Whether to fix upgrade friction before shipping more features. Measured 2026-09-20: the median package has 41% of installs on its latest release, but wdk-wallet has 2% and wdk-wallet-evm 1%, which points at the exact pins rather than at anything readers are doing.",
    verdict: "add",
  },
  ...
  // ---- Extend: did others build on it? ----------------------------------------------------
  {
    ...
    tip: "Shipped modules on the map published by someone else.",
  ...
  {
    id: "third-party-downloads",
    tip: "Tarball fetches of packages others publish, last complete period.",
    label: "Downloads of third-party packages",
    question: "Are the modules others built being used, or only built?",
    definition:
      "Downloads of every package a third party publishes for a module on the map, summed over the last complete period. Every tarball fetch counts. Kept apart from ours: no first-party figure or key result includes them.",
    source: "npm registry, api.npmjs.org",
    window: "last complete day, week or month; the current period is left out",
    floor: 50,
    canFall: true,
    decision:
      "Which outside modules the documentation and the examples should point at, and whether the module interface deserves more investment or the modules exist without anyone installing them.",
    verdict: "add",
  },
  {
    id: "external-prs-merged",
    tip: "Pull requests merged from authors outside the org, per period.",
  ...
  {
    id: "external-authors",
    tip: "Distinct outside authors of pull requests, bots excluded.",
  ...
  {
    id: "issue-response",
    tip: "Issues answered within seven days, over the last 30 days.",
  ...
  {
    id: "open-issues",
    tip: "Issues open across the selection on the day of collection.",
    label: "Open issues",
  ...
  {
    id: "prs-flow",
    tip: "Pull requests opened and merged per complete period.",
    label: "Pull requests",
    question: "Is work moving through review, or piling up in it?",
    definition: "Pull requests opened and pull requests merged in each complete period, across the selection, whoever wrote them.",
    source: "GitHub API",
    window: "last complete periods",
    floor: 10,
    canFall: true,
    decision:
      "Whether the constraint is writing the code or reviewing it. Opened far above merged for several periods points at review capacity, not at effort.",
    verdict: "keep",
  },
  {
    id: "issues-flow",
    tip: "Issues opened and closed per complete period.",
  ...
  // ---- Inventory: what we own. True, but not outcomes. -------------------------------------
  {
    ...
    tip: "Selected repos with a package on npm, stable against beta.",
  ...
  {
    id: "contributors",
    tip: "Distinct commit authors across the selection, bots excluded.",
    label: "Contributors",
  ...
  {
    id: "stars",
    tip: "GitHub stars summed across the selection, on the day.",
    label: "GitHub stars",
  ...
  {
    id: "atlas-counts",
    tip: "Rows counted in atlas.yaml, not measured anywhere.",
  ...
  {
    id: "readiness",
    tip: "Per package: stable tag, green CI, audit, health, signer, owners.",
...
export const METRICS = Object.freeze(checkVerdicts(defs).map(Object.freeze));
