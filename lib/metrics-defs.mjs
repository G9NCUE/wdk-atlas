...
const defs = [
  ...
  },
  {
    ...
    definition:
      "Downloads of every package a third party publishes for a module on the map, summed over the last complete period. Counts every tarball fetch. Nothing in WDK pins these packages, so there is no induced part to take out, and they are kept apart from ours: no first-party figure or key result includes them.",
    source: "npm registry, api.npmjs.org; each package named by its repository's manifest, read through GitHub",
    window: "last complete day, week or month; npm reports a few days late so the current period is left out",
    floor: 50,
    canFall: true,
    decision:
